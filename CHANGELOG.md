...
- `Server.newSession`, `newWindow`, `split` and `splitWindow` accept
  ...
  explicit `at index:` placement; an occupied index fails. Every value
  reaches tmux escaped, so one ending in `;` cannot split into a second
  command. (#12)
...
- **Breaking.** `WorkspaceBuilderError` gained a `callback(any Error)` case,
  ...
  tmux invocation failure, and is no longer `Hashable` because that error cannot
  be compared. (#12)
...
- `WorkspaceBuilder.build` waits for a newly built pane's shell to draw its
  ...
  redraw it a second time. An unreadable pane's command still sends right away.
  (#12)
