...
line that arrives after the wait starts — a daemon coming up, which is the case
this is for:
...
Only the right column is a measurement. Polling's cost is a function of how
fast the machine gets round its loop — three runs of the benchmark saw 31, 35
and 37 processes for the same two-second wait — and a number that moves is not
a claim worth writing down.
