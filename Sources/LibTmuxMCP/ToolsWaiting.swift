...
extension TmuxTools {
    func waitForOutput(_ arguments: Arguments) async throws -> ToolOutcome {
        ...
        let result = try await server.waitForOutput(
            in: pane,
            matching: try arguments.optionalStrings("patterns") ?? [],
            stoppingAt: try arguments.strings("stops"),
            requiringFreshOutput: try arguments.bool("require_fresh", or: false),
            timeout: timeout
        )
        return .init(OutputWaitResult(result, effectiveTimeout: enforced))
    }

    func watchFormat(_ arguments: Arguments) async throws -> ToolOutcome {
        ...
        let outcome = try await server.connected(attachingTo: session) { _, control in
    ...
    func waitForChannel(_ arguments: Arguments) async throws -> ToolOutcome {
        ...
        let released = await withTaskGroup(of: Bool.self) { group in
            ...
            // Cancelling the wait is what keeps a timeout from leaving a tmux
            // process blocked on a channel nobody will ever signal.
            group.cancelAll()
            return first
        }
        return .init(
